<script lang="ts">
  ...
  async function selectAssetsEditor() {
    ...
    isLaunching = true;
    try {
      await invoke(COMMANDS.SET_TIBIA_BASE_PATH, {
    ...
    try {
      ...
      if (!forceSelect) {
        try {
          ...
          console.error(
            "Failed to select monster folder after retry:",
            innerError,
          );
        }
      }

      showStatus(translate("launcher.error.monsterPath"), "error");
    } finally {
  ...
  async function selectNpcEditor(forceSelect = false) {
    ...
    try {
      ...
      showStatus(translate("launcher.error.npcPath"), "error");
    } finally {
      isLoadingNpc = false;
    }
  }
  ...
  <div class="app-launcher-card">
    <div class="launcher-view">
      ...
      <section class="launcher-hero">
        <h1 class="launcher-title">{translate("launcher.hero.title")}</h1>
        <p class="launcher-subtitle">{translate("launcher.hero.subtitle")}</p>
      </section>

      <!-- Tibia Path -->
      <section class="launcher-tibia-path-section">
        <h3 class="launcher-section-title">
          {translate("launcher.tibia.title")}
        </h3>
        <p class="launcher-secondary-text">
          {translate("launcher.tibia.subtitle")}
        </p>
        <div class="launcher-path-input-group">
          <label class="launcher-path-label" for="tibia-path-input"
            >{translate("launcher.tibia.label")}</label
          >
      ...
      <div class="launcher-options-grid">
        ...
        <button
          ...
          title={!appState.tibiaPath
            ? translate("launcher.assets.tooltip.noPath")
            : ""}
        >
          <div class="launcher-option-icon">🗂️</div>
          <h3>{translate("launcher.assets.title")}</h3>
          <p>{translate("launcher.assets.desc")}</p>
          <span class="launcher-option-badge">
            {isLaunching
              ? translate("launcher.status.loading")
              : translate("launcher.status.current")}
          </span>
        </button>
        ...
        <button
          ...
          title={translate("launcher.monster.tooltip.force")}
        >
          <div class="launcher-option-icon">👾</div>
          <h3>{translate("launcher.monster.title")}</h3>
          <p>{translate("launcher.monster.desc")}</p>
          {#if appState.monsterPath}
            <span class="launcher-option-badge active"
              >{translate("launcher.status.ready")}</span
            >
        ...
        >
          <div class="launcher-option-icon">🧙</div>
          <h3>{translate("launcher.npc.title")}</h3>
          <p>{translate("launcher.npc.desc")}</p>
          {#if appState.npcPath}
            <span class="launcher-option-badge active"
              >{translate("launcher.status.ready")}</span
            >
      ...
      <section class="launcher-configurations">
        ...
        <div class="config-group">
          <label for="theme-select">{translate("launcher.config.theme")}</label>
          <select
            id="theme-select"
            class="config-select"
        ...
        <div class="config-group">
          <label for="lang-select"
            >{translate("launcher.config.language")}</label
          >
          <select
            id="lang-select"
            class="config-select"
            bind:value={settingsState.language}
          >
        ...
        <!-- Refresh -->
        <div class="config-group">
          <label for="refresh-btn"
            >{translate("launcher.config.maintenance")}</label
          >
          <button
            id="refresh-btn"
            type="button"
          ...
          >
            ...
            {translate("launcher.config.refresh")}
          </button>
        </div>
      </section>
    </div>
